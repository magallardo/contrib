...
import (
	...
	"github.com/magallardo/contrib/activity/sqlupdate/util"
...
func New(ctx activity.InitContext) (activity.Activity, error) {
	...
	if sqlStatement.Type() != util.StUpdate {
		return nil, fmt.Errorf("only update statement is supported")
	}
...
func (a *Activity) Cleanup() error {
	if a.stmt != nil {
		err := a.stmt.Close()
		log.RootLogger().Warnf("error cleaning up SQL Update activity: %v", err)
	}

	log.RootLogger().Tracef("cleaning up SQL Update activity")
...
func (a *Activity) Eval(ctx activity.Context) (done bool, err error) {
	...
	results, err := a.doUpdate(in.Params)
	if err != nil {
		return false, err
	}
...
func (a *Activity) doUpdate(params map[string]interface{}) (interface{}, error) {
